...
public sealed class LdrLoadDllStubInjector : DllInjector
{
	...
	public override IntPtr Inject(string text)
	{
		...
		try
		{
			...
			base.WaitForRemoteThreadCompletion(intPtr3, "LdrLoadDll stub");
	...
	internal IntPtr BuildLoaderStub(IntPtr address, string text, out int intValue, out int intValue2)
	{
		...
		byte[] bytes2 = null;
		if (!PlatformInfo.flag8 && !PlatformInfo.flag7 && !PlatformInfo.flag6 && PlatformInfo.flag3)
		{
			string secondaryPath = string.Concat(new string[]
			{
			...
			bytes2 = Encoding.Unicode.GetBytes(secondaryPath);
			if (bytes2.Length > ushort.MaxValue)
			{
				throw new ArgumentException("The generated loader path is too long for a remote UNICODE_STRING.", nameof(text));
			}
		}

		long allocationSize = checked(4096L + bytes.Length + (bytes2?.Length ?? 0));
