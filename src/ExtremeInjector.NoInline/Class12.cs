...
public sealed class Class12
{
	...
	{
		if (!smethod_2(string_2))
		{
			return new Class12();
		}
		try
		{
			DataContractSerializer xmlObjectSerializer_ = smethod_4(smethod_3(typeof(Class12).TypeHandle));
			XmlReader xmlReader = smethod_5(string_2);
			try
			{
				return (Class12)smethod_6(xmlObjectSerializer_, xmlReader);
			}
			finally
			{
				if (xmlReader != null)
	...
	{
		DataContractSerializer xmlObjectSerializer_ = smethod_4(smethod_3(typeof(Class12).TypeHandle));
		string string_ = Class178.smethod_0(102);
		XmlWriterSettings xmlWriterSettings_ = smethod_8();
		smethod_9(xmlWriterSettings_, bool_0: true);
		XmlWriter xmlWriter = smethod_10(string_, xmlWriterSettings_);
		try
		{
			smethod_11(xmlObjectSerializer_, xmlWriter, class12_0);
		}
		finally
		{
			if (xmlWriter != null)
